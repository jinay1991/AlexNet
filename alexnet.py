...
def AlexNet(features, net_data=None, rate=1.0, features_extract=False):
    mu = 0
    sigma = 1e-2

    # Input ?x?x3 Output 227x227x3
    with tf.compat.v1.variable_scope('P0'):
        p0 = tf.image.resize(features, (227, 227))
    ...
    with tf.compat.v1.variable_scope('C1'):
        if net_data:
            ...
            weight1 = tf.Variable(tf.random.truncated_normal(shape=(11, 11, 3, 96), mean=mu, stddev=sigma))
        ...
        conv1 = tf.nn.dropout(conv1, rate=rate)
    ...
    with tf.compat.v1.variable_scope('S1'):
    ...
    with tf.compat.v1.variable_scope('C2'):
        if net_data:
            weight2 = tf.Variable(net_data['conv2'][0])
            bias2 = tf.Variable(net_data['conv2'][1])
        else:
            weight2 = tf.Variable(tf.random.truncated_normal(shape=(5, 5, 48, 256), mean=mu, stddev=sigma))
        ...
        conv2 = tf.nn.dropout(conv2, rate=rate)
    ...
    with tf.compat.v1.variable_scope('S2'):
    ...
    with tf.compat.v1.variable_scope('C3'):
        weight3 = tf.Variable(tf.random.truncated_normal(shape=(3, 3, 256, 384), mean=mu, stddev=sigma))
        ...
        conv3 = tf.nn.dropout(conv3, rate=rate)

    # Input 13x13x384 Output 13x13x384
    with tf.compat.v1.variable_scope('C4'):
        if net_data:
            weight4 = tf.Variable(net_data['conv4'][0])
            bias4 = tf.Variable(net_data['conv4'][1])
        else:
            weight4 = tf.Variable(tf.random.truncated_normal(shape=(3, 3, 192, 384), mean=mu, stddev=sigma))
        ...
        conv4 = tf.nn.dropout(conv4, rate=rate)

    # Input 13x13x384 Output 13x13x256
    with tf.compat.v1.variable_scope('C5'):
        if net_data:
            weight5 = tf.Variable(net_data['conv5'][0])
            bias5 = tf.Variable(net_data['conv5'][1])
        else:
            weight5 = tf.Variable(tf.random.truncated_normal(shape=(3, 3, 192, 256), mean=mu, stddev=sigma))
        ...
        conv5 = tf.nn.dropout(conv5, rate=rate)

    # Input 13x13x256 Output 6x6x256
    with tf.compat.v1.variable_scope('S3'):
    ...
    with tf.compat.v1.variable_scope('FC5'):
    ...
    with tf.compat.v1.variable_scope('FC6'):
        if net_data:
            ...
            weight6 = tf.Variable(tf.random.truncated_normal(shape=(9216, 4096), mean=mu, stddev=sigma))
        ...
        fc6 = tf.nn.dropout(fc6, rate=rate)
    ...
    with tf.compat.v1.variable_scope('FC7'):
        if net_data:
            ...
            weight7 = tf.Variable(tf.random.truncated_normal(shape=(4096, 4096), mean=mu, stddev=sigma))
        ...
        fc7 = tf.nn.dropout(fc7, rate=rate)
    ...
    with tf.compat.v1.variable_scope('FC8'):
        if net_data:
            ...
            weight8 = tf.Variable(tf.random.truncated_normal(shape=(4096, 1000), mean=mu, stddev=sigma))
...
def retrain(X_train, y_train, net_weights=None, learning_rate=0.001, epochs=10, batch_size=128, save_graph="alexnet.pb"):
    ...
    fc7 = AlexNet(features, net_data=net_data, features_extract=True, rate=0.5)
    if net_data:
        fc7 = tf.stop_gradient(fc7)
    with tf.compat.v1.variable_scope('fc8'):
        weight8 = tf.Variable(tf.random.truncated_normal(shape=(fc7.get_shape().as_list()[-1], nclasses), mean=0, stddev=1e-2))
    ...
    with tf.compat.v1.Session() as sess:
        ...
        output_graph_def = tf.compat.v1.graph_util.convert_variables_to_constants(sess, tf.get_default_graph().as_graph_def(), ["fc8/Add"])

        with tf.compat.v1.gfile.GFile(os.path.join(model_dir, save_graph), "wb") as f:
